...
jQuery( function( $ ) {
	...
	try {
			var woo_ch_types=['customer','product'];
			$( document.body ).on( 'wc-sinic-enhanced-select-init', function() {
				
				// Ajax search boxes
				function on_obj_search(obj_type){
					$( ':input.wc-sinic-'+obj_type+'-search' ).filter( ':not(.enhanced)' ).each( function() {
						var select2_args = {
				...
				for(var i=0;i<woo_ch_types.length;i++){
					on_obj_search(woo_ch_types[i]);
				}
			})
			...
			$( 'html' ).on( 'click', function( event ) {
				if ( this === event.target ) {
					for(var i=0;i<woo_ch_types.length;i++){
						$( ':input.wc-sinic-'+woo_ch_types[i]+'-search' ).filter( '.select2-hidden-accessible' ).select2( 'close' );
					}
				}
			});
