 ...
 * @author ranj
 */
abstract class Abstract_WC_Sinic_Settings {
	/**
	 * The plugin ID.
	...
	public function admin_form_start(){
    	    if( isset($_POST['notice-'.$this->id])){
    	        if(wp_verify_nonce($_POST['notice-'.$this->id], WC_Sinic::instance()->session->get_notice('admin:form:'.$this->id,true))){
    	           $this->process_admin_options(); 
    	           
    	        }else{
    	            $this->errors[]=WC_Sinic_Error::err_code(701)->errmsg;
    	            $this->display_errors();
    	        }
    	    }
	...
	public function admin_options() {	
            ...
            <?php do_action('woo_ch_admin_options_header_'.$this->id)?>
	...
	public function admin_form_end(){
    	   ?><p class="submit">
    			<input type="hidden" name="notice-<?php print $this->id?>" value="<?php print wp_create_nonce ( WC_Sinic::instance()->session->get_notice('admin:form:'.$this->id));?>"/>
	...
	public function get_form_fields() {
		return apply_filters ( 'woo_ch_settings_api_form_fields_' . $this->id, $this->form_fields );
	}
	/**
	 * 获取页面下拉选项
	 * @return array
	...
	public function process_admin_options() {
		...
		do_action('woo_ch_process_admin_options_validate_'.$this->id);
	...
	public function get_post_type_options(){
	    ...
	    foreach ($wp_post_types as $key=>$type){
	        ...
	        if($type->show_ui&&$type->public&&!(isset($type->woo_ch_ignore)&&$type->woo_ch_ignore)){
	            $types[$key]=isset($type->label)&& !empty($type->label)?"{$type->label}({$key})":$key;
	        }
	    }
	    return apply_filters('woo_ch_post_types', $types);
	}
	        ...
	        <tr valign="top" class="<?php echo isset($data['tr_css'])?$data['tr_css']:''; ?>">
	        	...
	        	<td class="forminp">
	        		<fieldset>
						...
						<input type="button" class="button" id="btn-<?php echo esc_attr( $field ); ?>-upload-img" value="<?php echo __('Upload Image',WC_SINIC)?>" />
						<a href="javascript:void(0);" style="margin-left:5px;" id="btn-<?php echo esc_attr( $field ); ?>-remove"><?php echo __('Remove',WC_SINIC)?></a>
						<script type="text/javascript">
						(function($){
						    ...
						    $('#btn-<?php echo esc_attr( $field ); ?>-remove').click(function(){
								if(confirm('<?php echo __('Are you sure?',WC_SINIC)?>')){
	...
	/*public function generate_img_html($key, $data){
	    ...
	    $default_val=array();
	    if(!empty($default)){
	        $default_val = WC_Sinic::instance()->generate_request_params(array(
	            'url'=>$data['default']
	        ));
	    }
	    ...
	    if(!defined('xh_webuploader')){
	    	...
	    	<script type="text/javascript" src="<?php echo WC_SINIC_URL?>/assets/webuploader-0.1.7-alpha/webuploader.js"></script>
	        <?php 
	    }
		...
		<div class="xh-form-group" id="form-group-<?php echo $input_id?>">
            ...
            <div>
            	...
            	<div id="image-<?php print $input_id;?>-picker"><?php echo __('Upload image',WC_SINIC)?></div>
        ...
        <script type="text/javascript">
        	(function($){
        		...
        		var <?php print $input_id;?>_config ={
        			swf:'<?php echo WC_SINIC_URL.'/assets/webuploader-0.1.5/Uploader.swf'?>',
        			server:'<?php echo WC_SINIC::instance()->ajax_url(array('action'=>"xh_uc_upload_img",'w'=>$img_size['width'],'h'=>$img_size['height']),true,true)?>',
        		...
        		<?php print $input_id;?>_uploader.on( 'uploadStart', function( file ) {
        			$('#image-<?php print $input_id;?>-picker .webuploader-pick').text('<?php echo __('Uploading...',WC_SINIC)?>');
        		});
        		// 文件上传失败，显示上传出错。
        		<?php print $input_id;?>_uploader.on( 'uploadError', function( file ,reason) {
        			<?php print $input_id;?>_uploader.reset();
        			alert('<?php echo WC_SINIC_Error::err_code(500)->errmsg?>');
        		});
        
        		// 完成上传完了，成功或者失败，先删除进度条。
        		<?php print $input_id;?>_uploader.on( 'uploadComplete', function( file ) {
        			$('#image-<?php print $input_id;?>-picker .webuploader-pick').text('<?php echo __('Upload image',WC_SINIC)?>');
        		});
        		...
        		<?php print $input_id;?>_uploader.on( 'uploadSuccess', function( file ,response) {
        			...
        			if(!response||typeof response.errcode=='undefined'){
        				alert('<?php echo WC_SINIC_Error::error_unknow()->errmsg?>');
        				return;
        			}
                ...
                <tr valign="top" class="<?php echo isset($data['tr_css'])?$data['tr_css']:''; ?>">
                	...
                	<td class="forminp">
                		<fieldset>
                		    ...
                		    ?>
                			<select class="wc-sinic-search" data-multiple="<?php echo $multiple?>" data-custom_params="<?php echo $custom_params?esc_attr(json_encode($custom_params)):null; ?>" data-type='<?php echo $data['post_type'];?>' name="<?php echo esc_attr( $field ); ?>" id="<?php echo esc_attr( $field_key ); ?>" data-sortable="true" data-placeholder="<?php echo __( 'Search...', WC_SINIC); ?>" data-allow_clear="true">
                    			<?php 
                    			$vals=array();
                    		...
                    		<script type="text/javascript">
                        		(function($){
    								$(document).bind('wc-sinic-on-select2-inited',function(){
