...
abstract class WC_Sinic_Object{
    ...
    public function get($property,$default = null){
        return apply_filters("woo_ch_{$this->get_table_name()}_property_{$property}", isset($this->{$property})?$this->{$property}:$default);
    }
    ...
    public function insert(){
        ...
        if(!empty($wpdb->last_error)){
            WC_Sinic_Log::error($wpdb->last_error);
            return WC_Sinic_Error::error_custom($wpdb->last_error);
        }
        
        if(!$result){
            WC_Sinic_Log::error("插入数据时发生异常:table:{$table_name},errors:{$wpdb->last_error},datas:".print_r($data,true));
            return WC_Sinic_Error::error_unknow();
        }
    ...
    */
    public function update($property_valuess=array(),&$result =null){
        ...
        if(!empty($wpdb->last_error)){
            WC_Sinic_Log::error($wpdb->last_error);
            return WC_Sinic_Error::err_code($wpdb->last_error);
        }
    ...
    public function save_or_update(){
        ...
        if($this->get_by($primary_key, $this->{$primary_key})){
            ...
            if(!empty($wpdb->last_error)){
                WC_Sinic_Log::error($wpdb->last_error);
                return WC_Sinic_Error::err_code(500);
            }
        }else{
            $result = $wpdb->insert($table_name, $data);
            if(!empty($wpdb->last_error)){
 ...
 * @TODO 此类已作废
 */
abstract class WC_Sinic_Mixed_Object extends WC_Sinic_Object{
...
abstract class WC_Sinic_Post_Object extends WC_Sinic_Object{
    /**
     * 
     * @var int
     */
    ...
    /**
     * {@inheritDoc}
     * @see WC_Sinic_Object::is_auto_increment()
     */
    ...
    /**
     * {@inheritDoc}
     * @see WC_Sinic_Object::get_primary_key()
     */
 ...
 * TODO:此类已作废
 */
class WC_Sinic_Mixed_Object_Factory{
    /**
     * TODO:此方法已作废
     * @param mixed $obj
     * @return WC_Sinic_Mixed_Object|NULL
     */
    public static function to_entity($obj){
        ...
        if(is_object($obj)){
            if(isset($obj->sn)){
                return new WC_Sinic_Order($obj);
            }
            
            if(isset($obj->post_ID)&&isset($obj->order_id)){
                return new WC_Sinic_Order_Item($obj);
            }
        }
