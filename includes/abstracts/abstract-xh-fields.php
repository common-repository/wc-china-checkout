...
abstract class Abstract_XH_WC_Sinic_Fields extends Abstract_WC_Sinic_Settings{   
    /**
    ...
    public function admin_init(){
        ...
        foreach ($this->post_types as $post_type=>$name){
            add_meta_box('wc-sinic-metabox-'.$this->id,
                $this->title,
    ...
    /**
     * {@inheritDoc}
     * @see Abstract_XH_WC_Sinic_Fields::meta_box_html()
     * @since 1.0.0
     */
    ...
    public function process_object_update($post){
        ...
        $error = apply_filters('woo_ch_process_object_update', $error,$product);
        return apply_filters("woo_ch_{$post->post_type}_process_object_update", $error,$product);
    }
    /**
     *
     * {@inheritDoc}
     * @see Abstract_XH_WC_Sinic_Fields::sale_meta_box_data()
     * @since 1.0.0
     */
    ...
    public function admin_options(){
        ...
        <input type="hidden" name="woo_ch_post_fields" value="<?php echo $this->id?>"/>
	...
	public function process_admin_options() {
	    ...
	    if (count ( $this->errors ) > 0) {
	        if($wp_error){
	            return WC_Sinic_Error::error_custom($this->errors[0]);
	        }
	        $this->display_errors ();
	        return false;
	    } 
        ...
        try {
            ...
            if(!WC_Sinic_Error::is_valid($error)){
                if($wp_error){
                    return $error;
                }
            ...
            foreach ($this->sanitized_fields as $key=>$val){
                update_post_meta($post->ID, "woo_ch_{$key}", $val);
            }
        } catch (Exception $e) {
            if($wp_error){
                return WC_Sinic_Error::error_custom($e->getMessage());
            }
        ...
        return $wp_error?WC_Sinic_Error::success():true;
	}
	...
	public function display_errors() {
	    ...
	    wp_die(__('Update failed,detail error:',WC_SINIC).join('<br/>',$this->errors));
	}
	
	/**
	 * 
	 * @param WP_Post $post
	 * @return WC_Sinic_Post_Object
	 */
